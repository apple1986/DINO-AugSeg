...
class KVASIR(Dataset):
    """ KVASIR Dataset for multi-object segmentation and tracking """
    ...
    def __init__(self, root, split='trainval', transform_img=None, img_size=224, train_num="all"):
        ...
        split_file = os.path.join(self._base_dir, f'dataset/Kvasir_SEG/{split}_dino.txt')
        ...
        self.abs_path = os.path.join(self._base_dir, "dataset/Kvasir_SEG")

    def __len__(self):
        return len(self.curIMG_list)

    def __getitem__(self, idx):
        sample_path = self.curIMG_list[idx] + ".jpg"  # e.g. "sequence01.npz"
        sample_path = sample_path.strip()
        img_path = os.path.join(self.abs_path, "images" ,sample_path)
        gt_path = os.path.join(self.abs_path, "masks" ,sample_path)
