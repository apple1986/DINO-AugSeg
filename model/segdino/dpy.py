...
if __name__ == "__main__":
    ...
    model_weight_path = os.path.join(root_path, "dino_seg/checkpoint/dinov3_vitb16_pretrain_lvd.pth") 
    backbone = torch.hub.load(REPO_DIR, 'dinov3_vitb16', source='local', weights=model_weight_path)
    dpt = DPT(nclass=2, backbone=backbone)
    # dpt = DPT(encoder_size='base', nclass=2, features=128, out_channels=[768, 768, 768, 768], backbone=model)
    dpt.lock_backbone()
