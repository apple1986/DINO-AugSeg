import sys
sys.path.append("/home/gxu/proj1/lesionSeg/dino_seg/model")
...
from dinov3_lora.lora import LoRA
from dinov3_lora.linear_decoder import LinearClassifier
from dinov3_lora.fpn_decoder import FPNDecoder
    ...
    def __init__(
        ...
        if self.use_fpn:
            self.decoder = FPNDecoder(
                ...
                out_channels=128,
                patch_h=int(img_dim[0] / self.patch_size),
                patch_w=int(img_dim[1] / self.patch_size),
                n_classes=n_classes,
            )
        else:
...
if __name__ == "__main__":
    ...
    model_weight_path = os.path.join(REPO_DIR, "checkpoint/dinov3_vitb16_pretrain_lvd.pth") 
    dinov3_convnext_tiny = torch.hub.load(REPO_DIR, 'dinov3_vitb16', source='local', weights=model_weight_path)
    ...
    logits = dino_lora(x)
    print(logits.shape)  # Expected output: (2, 4, 520, 520)