    ...
    **kwargs,
    ...
    if pretrained:
        if type(segmentor_weights) is SegmentorWeights:
            ...
            url = "/home/gxu/proj1/lesionSeg/dino_seg/checkpoint/dinov3_vit7b16_ade20k_head.pth"
        else:
        ...
        state_dict = torch.load(url, map_location="cpu") # ap
...
##########################: ap
def _make_dinov3_m2f_segmentor_ap(
    *,
...
def dinov3_vitb16_ms( 
    ...
    **kwargs,
    ...
    return _make_dinov3_m2f_segmentor_ap(
...
## ap: vit_s
def _make_dinov3_m2f_segmentor_vits(
    *,
    ...
    **kwargs,
    ...
    if backbone_name == "dinov3_vits16":
        backbone_model = dinov3_vits16(pretrained=pretrained, weights=backbone_weights, check_hash=check_hash)
    else:
        raise AssertionError(f"No pretrained segmentation checkpoint available for {backbone_name}")
    ...
    if pretrained:
        if type(segmentor_weights) is SegmentorWeights:
            ...
            model_filename = f"{backbone_name}_{segmentor_weights_name}_m2f_head-{hash}.pth"
            url = os.path.join(DINOV3_BASE_URL, backbone_name, model_filename)
            # url = "/home/gxu/proj1/lesionSeg/dino_seg/checkpoint/dinov3_vit7b16_ade20k_head.pth" # ap
        else:
            url = convert_path_or_url_to_url(segmentor_weights)
        state_dict = torch.hub.load_state_dict_from_url(url, map_location="cpu", check_hash=check_hash)
        # state_dict = torch.load(url, map_location="cpu") # ap
...
def dinov3_vits16_ms( 
    ...
    **kwargs,
    ...
    return _make_dinov3_m2f_segmentor_vits(
...
## ap: convnext_t: to be continue: need to be modify the 
def _make_dinov3_convnext_t_seg(
    *,
    ...
    **kwargs,
    ...
    if backbone_name == "dinov3_convnext_tiny":
        backbone_model = dinov3_convnext_tiny(pretrained=pretrained, weights=backbone_weights, check_hash=check_hash)
    else:
        raise AssertionError(f"No pretrained segmentation checkpoint available for {backbone_name}")
...
def dinov3_convnext_t_ms( 
    ...
    **kwargs,
    ...
    return _make_dinov3_convnext_t_seg(
        backbone_name="dinov3_convnext_tiny",
        pretrained=pretrained,
        weights=weights,
        backbone_weights=backbone_weights,
        check_hash=check_hash,
